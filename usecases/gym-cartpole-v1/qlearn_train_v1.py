...
def get_discrete_state(state):
    discrete_state = state/np_array_win_size + np.array([50,30,50,30])

    for i in [1,3]:
        if discrete_state[i]>=62:
            discrete_state[i] = 61 
...
Observation = [102, 62, 102, 62]
np_array_win_size = np.array([0.1, 0.1, 0.01, 0.1])


# Initiation
q_table = np.random.uniform(low=0, high=1, size=(Observation + [env.action_space.n]))
# q_table = np.load(PATH_MODEL+'qlearn_v1.npy') 
print(q_table.shape)
...
for i in range(1, 8000):
    ...
    if i % checkp == 0: 
        ...
        np.save(PATH_MODEL+'qlearn_v1.npy', q_table)
        epsilon = epsilon*(1-0.015)
        print('epsilon decay : ',round(epsilon,3))

print("Training finished.\n")
np.save(PATH_MODEL+'qlearn_v1.npy', q_table) 
np.save(PATH_MODEL+'mean_time_v1.npy', np.array(all_mean_time)) 
np.save(PATH_MODEL+'mean_reward_v1.npy', np.array(all_mean_reward)) 